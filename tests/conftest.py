...
def pytest_configure(config):
    ...
    print(f"\nRunning tests on {system} ({platform.platform()})")
    
    if system == 'Darwin':  # Mac
        print("Mac-specific test configuration loaded")
    elif system == 'Windows':
        print("Windows-specific test configuration loaded")
    else:
        print("Unix-like system test configuration loaded")
...
# Custom markers for platform-specific tests
def pytest_configure(config):
    config.addinivalue_line("markers", "mac: mark test as Mac-specific")
    config.addinivalue_line("markers", "windows: mark test as Windows-specific")  
    config.addinivalue_line("markers", "unix: mark test as Unix-specific")
    config.addinivalue_line("markers", "cross_platform: mark test as cross-platform")